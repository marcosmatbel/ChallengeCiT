    ...
    internal class DefaultColumnBuilder<T> : IColumnBuilder<T> where T : class
    {
        ...
        private IGridColumn<T> CreateColumn(PropertyInfo pi, bool hidden)
        {
            ...
            if (!hidden && column != null)
            {
                ...
                column.Export(DefaultExportEnabled);
            }
            return column;
        }
        ...
        {
            column.Encoded(options.EncodeEnabled).Sanitized(options.SanitizeEnabled).Export(options.ExportEnabled);
            if (!string.IsNullOrEmpty(options.Format))
                column.Format(options.Format);
        }
    }
